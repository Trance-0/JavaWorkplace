...
//			pre = i;
//		}
		container[index] = getLongestLine(points);
		index++;
		return container;
	}

	private LineSegment getLongestLine(Point[] points) {
		Point largest = points[0];
		Point lowest = points[1];
		for (int i = 2; i < points.length; i++) {
			if (largest.compareTo(points[i]) < 0) {
				largest = points[i];
			}
			if (lowest.compareTo(points[i]) > 0) {
				lowest = points[i];
			}
		}
		return new LineSegment(largest, lowest);
	}
...
//	}
	...
	public static void main(String[] args) {
		Point a = new Point(1000, 18000);
		Point b = new Point(2000, 22000);
		Point c = new Point(3000, 26000);

		Point i = new Point(6991, 13751);
		Point d = new Point(3500, 28000);
		Point e = new Point(4000, 30000);
		Point f = new Point(4500, 32000);

		Point g = new Point(3247, 13379);
		Point h = new Point(4183, 13472);
		Point j = new Point(9799, 14030);
